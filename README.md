...
- ⭐ **收藏**:把「路線 + 車站」加入收藏,首頁直接顯示即時 ETA(存於 localStorage);有收藏時首頁收藏排第一,可喺設定 ↕️ 排次序。
- 🚶 **趕唔趕到**:每個收藏可設步行時間,標示「趕唔切」/「🚶 搭呢班」同「最遲 HH:MM 出門」;附近分頁按距離自動估步行時間。
- 🚇 **港鐵收藏**:港鐵時間表撳 ☆ 收藏站 + 方向(最多 4 個),首頁直接見下兩班車同月台;鐵路分頁記住上次揀嘅綫同站。
- ⇄ **一撳返程**:路線頁轉對面方向並自動打開最近嘅對面站;開路線會自動捲去你嘅站。
- 📍 **自動打開最近你嘅站**:由搜尋開一條線,會用你嘅位置搵呢條線離你最近、上得車嘅站(1 公里內),自動展開到站時間並捲過去,地圖用藍點標你嘅位置;太遠就只提示唔會亂跳,可喺設定(⚙️)關。位置只喺你部機計,唔會上傳。
- 📶 **弱網友善**:網絡唔穩時保留上次到站(最多 5 分鐘)並標示,唔會一斷線就清空;班次備註轉做「預定」「尾班車」標籤。
- 🌙 **深色模式**:一鍵切換,記住偏好。
- 🐼 **會郁嘅公仔**:原創熊貓 + 啡熊識眨眼、揮手;門口顯示模式有車就到會跳、夜晚會眼瞓;集印仔解鎖蝴蝶結、車長帽、可可卡等造型。UI 卡片 / 頁面 / 班次有輕量彈入滑入動畫(尊重「減少動態效果」)。
- 📺 **門口顯示模式**(iPad 橫擺 kiosk):大字時鐘 + 收藏路線實時到站(10 秒刷新,橫屏 2×3 最多 6 個站)+ 是日勵志名句 + 天氣 + 香港新聞輪播 + 雙公仔;19:00–07:00 自動深色,wake lock 防瞓,`#display` 直達;**畫面鎖定**(誤觸只彈提示,長按 3 秒先退出),iOS「加入主畫面」後全屏冇 Safari bar。金句可喺設定揀:每日自動轉/固定一句/自己寫。設定(⚙️)開啟。
- 🔙 **返回鍵**:Android 返回鍵 / 瀏覽器上一頁 / 邊緣滑動,會逐層退返上一個畫面(揀地點 → 路線詳情 → 分頁 → 首頁),唔會一撳就閂咗成個 App;門口顯示模式已鎖定,撳返回只彈提示,要長按 3 秒先退出。
- 📲 **PWA**:可「加入主畫面」當 App 用,離線可開啟;部署新版後舊分頁唔會爆 chunk 404,有新版本會提示「撳一下更新」。
...
- 部署為純靜態檔案
- 所有定時刷新(到站 / 附近 / 港鐵 / 路況 / 路線地圖)喺分頁轉去背景時自動暫停,返嚟即刻補一次,上一輪未返唔會疊請求(`src/hooks/usePolling.ts`)
...
- 規劃頁嘅地址搜尋會將你輸入嘅文字傳送到香港政府 ALS 地址查詢服務;ALS 冇結果時後備用 komoot Photon(第三方)。地圖底圖同地名標籤由地政總署(mapapi.geodata.gov.hk)提供,瀏覽器會直接向佢哋請求圖塊。
