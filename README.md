...
查香港 **九巴 (KMB) / 龍運 (LWB) / 城巴 (CTB)** 實時到站時間的純前端 Web App。
資料來自運輸署開放數據(data.gov.hk),由 browser 直接呼叫,**無需自建後端、無需 API key**。

> 營辦商:搜尋會同時顯示九巴同城巴(路線號可能重複,以紅色 / 青色 badge 區分)。
> 城巴 API 無「全線一次 ETA」endpoint,所以**城巴路線冇預測巴士公仔**,只有路線地圖 + 逐站到站時間。「附近車站」目前只涵蓋九巴。
...
- 📍 **附近車站 (GPS)**:用定位找出最近車站(地圖 + 清單),展開即見該站所有路線的即時到站。
