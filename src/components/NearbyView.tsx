...
import { nearbyBuses, readNearbyCache, writeNearbyCache, type NearbyCo, type NearbyRow } from '../lib/nearby'
...
export default function NearbyView({
  ...
  const refresh = useCallback(async (c: NearbyCo, loc: { lat: number; lng: number }) => {
    try {
    ...
    <div>
      <div className="co-filter">
        ...
        <button
          ...
          style={
            tab === 'fit'
              ? { background: FIT_COLOR, borderColor: FIT_COLOR }
              : { color: FIT_COLOR, borderColor: FIT_COLOR }
          }
          onClick={() => setTab('fit')}
        >
          🏋️ 24/7
      ...
      {tab !== 'fit' && (
        <>
          {stale && (
            <div className="muted small" style={{ margin: '0 2px 8px' }}>
              ⏳ 顯示緊上次結果,更新緊…
            </div>
          )}
          ...
          {error && rows.length === 0 && (
            <div>
              ...
              <div style={{ textAlign: 'center' }}>
                <button className="primary-btn" onClick={() => void relocate()}>
                  重試
                </button>
              </div>
            </div>
